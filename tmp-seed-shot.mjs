// TEMP verification — seed selection, generate, capture the premium results experience.
...
const MAINS = ['BUSI1302', 'GİTA1106', 'ELEC2207', 'İYAR1123'];
...
(async () => {
  ...
  try {
    ...
    // results overlay auto-opens after generation
    await waitFor(cdp, "/Oluşturulan Programlar/.test(document.body.innerText)", { label: 'results overlay', timeout: 30000 });
    await pause(2200);
    await shot(cdp, 'results-top.png');

    // scroll the overlay to the shareable card
    ...
    await pause(1500);
    await shot(cdp, 'results-shareable.png');

    // reset scroll, then build a comparison
    await evaluate(cdp, `(() => { const el=[...document.querySelectorAll('div')].find(d=>typeof d.className==='string' && d.className.includes('fixed')&&d.className.includes('overflow-y-auto')); if(el){ el.scrollTop=0; } })()`);
    await pause(500);
    await evaluate(cdp, `(() => { const cmp=[...document.querySelectorAll('button')].filter(b=>(b.textContent||'').trim()==='Karşılaştır'); if(cmp[0])cmp[0].click(); if(cmp[1])cmp[1].click(); })()`);
    await pause(600);
    await evaluate(cdp, `(() => { const cmp=[...document.querySelectorAll('button')].filter(b=>(b.textContent||'').trim()==='Karşılaştır' && !b.disabled); if(cmp.length) cmp[cmp.length-1].click(); })()`);
    await waitFor(cdp, "/Karşılaştırma/.test(document.body.innerText)", { label: 'compare modal', timeout: 8000 }).catch(() => {});
    await pause(1200);
    await shot(cdp, 'results-compare.png');
  } catch (e) { console.error('ERR:', e.message); try { await shot(cdp, 'results-error.png'); } catch {} }
